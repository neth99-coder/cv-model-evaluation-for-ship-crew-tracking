...
def run_evaluation(
    ...
    """
     1. Enroll all reference images per person (``test/<name>/*.jpg`` etc).
    ...
    for person_dir in sorted(test_dir.iterdir()):
        ...
        success_for_person = 0
        for ref_image in images:
            ok = model.enroll(person_dir.name, str(ref_image))
            status = "OK " if ok else "FAIL"
            print(f"  [{status}] {person_dir.name} ← {ref_image.name}")
            if ok:
                success_for_person += 1
                enrolled_images += 1

        if success_for_person > 0:
            enrolled_people += 1

    print(
        f"  Enrolled {enrolled_people} person(s) "
        f"using {enrolled_images} reference image(s).\n"
    )
    ...
    metrics["enrolled_count"] = enrolled_people
    metrics["enrolled_images"] = enrolled_images
