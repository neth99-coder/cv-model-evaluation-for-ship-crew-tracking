...
class BaseFaceRecognizer(ABC):
    ...
    def __init__(self) -> None:
        # Store multiple embeddings per person to learn pose/lighting variation.
    ...
    def enroll(self, person_name: str, image_path: str) -> bool:
        ...
        if embedding is not None:
            self.database.setdefault(person_name, []).append(embedding)
            return True
    ...
    def _match_embedding(self, embedding: np.ndarray) -> tuple[str, float]:
        ...
        for name, db_embs in self.database.items():
            if not db_embs:
                continue
            scores[name] = max(cosine_similarity(embedding, db_emb) for db_emb in db_embs)
